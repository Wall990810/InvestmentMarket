...
/**
 * AI智能体框架整体集成测试
 * <p>验证从配置解析 → Agent创建 → 组件组装 → 消息处理 → 记忆存储的完整流程</p>
 */
    ...
    class EndToEndFlowTest {
        ...
        void fullFlow_configToConversation() {
            ...
            Skill echoSkill = new Skill() {
                @Override public String getName() { return "echo-skill"; }
                @Override public String getDescription() { return "Echo skill"; }
                @Override public String execute(String input) { return "Echo: " + input; }
            };
            skillRegistry.register(echoSkill);

            Tool mockTool = new Tool() {
                @Override public String getName() { return "mock-tool"; }
                @Override public String getDescription() { return "Mock tool"; }
                @Override public Map<String, Object> getParameterSchema() { return Map.of(); }
                @Override public String execute(Map<String, Object> parameters) { return "tool-result"; }
            };
            toolRegistry.register(mockTool);

            // 3. 创建Agent工厂并创建Agent
            MemoryStoreFactory memFactory = storeType -> memoryStore;
            AgentFactory factory = new AgentFactory(skillRegistry, toolRegistry, memFactory);
    ...
    class LifecycleIntegrationTest {
        ...
        void lifecycleManager_shouldManageFullLifecycle() {
            ...
            lifecycleManager.addListener(new AgentLifecycleListener() {
                @Override public void onCreated(Agent agent) { events.add("created:" + agent.getName()); }
                @Override public void onStarted(Agent agent) { events.add("started:" + agent.getName()); }
                @Override public void onStopped(Agent agent) { events.add("stopped:" + agent.getName()); }
                @Override public void onDestroyed(Agent agent) { events.add("destroyed:" + agent.getName()); }
            });

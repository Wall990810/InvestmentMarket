...
class MicrometerAgentMonitorTest {
	...
	class TraceLifecycleTest {
		...
		void traceEnd_shouldRecordSuccessAndDuration() {
			...
			var timer = meterRegistry.find("ai.agent.duration").tag("agent", "agent").timer();
			assertNotNull(timer);
	...
	class ToolCallTraceTest {
		...
		void traceToolCall_shouldRecordToolMetrics() {
			...
			var toolCounter = meterRegistry.find("ai.tool.calls").tag("tool", "calculator").counter();
			assertNotNull(toolCounter);
			assertEquals(1.0, toolCounter.count());

			var toolTimer = meterRegistry.find("ai.tool.duration").tag("tool", "calculator").timer();
	...
	class CustomMetricTest {
		...
		void recordMetric_shouldRegisterGauge() {
			...
			var gauge = meterRegistry.find("custom.metric").tag("env", "test").gauge();
			assertNotNull(gauge);

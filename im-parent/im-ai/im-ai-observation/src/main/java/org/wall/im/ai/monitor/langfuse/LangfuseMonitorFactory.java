...
/**
 * LangfuseMonitor工厂类
 * <p>提供LangfuseMonitor的默认配置入口，简化初始化和配置流程</p>
 *
 * <h3>使用示例</h3>
 * <pre>{@code
 * // 方式1: 使用环境变量（推荐）
...
public final class LangfuseMonitorFactory {
     ...
     * <p>默认连接到本地Langfuse实例 (http://localhost:3000)</p>
     *
    ...
    public static LangfuseMonitor fromEnvironment(AgentMonitor delegate) {
        ...
        config.setEnabled(publicKey != null && !publicKey.isEmpty()
     ...
     * <p>适用于需要自定义LangfuseClient配置（如自定义HTTP客户端、连接池等）的场景</p>
     *
    ...
    private static LangfuseClient buildClient(MonitorConfig.LangfuseConfig config) {
        var builder = LangfuseClient.builder()
                .url(config.getHost() != null ? config.getHost() : DEFAULT_HOST);

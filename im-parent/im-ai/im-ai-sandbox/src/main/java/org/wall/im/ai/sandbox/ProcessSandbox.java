...
/**
 * 进程隔离沙盒实现
 * <p>通过进程级别隔离限制Agent运行时的文件访问和执行环境</p>
 */
    ...
    public SandboxResult execute(String code, String workDir) {
        ...
        try {
            ...
            if (!finished) {
                ...
                return SandboxResult.failure("Execution timed out", -1,
    ...
    public SandboxResult executeCommand(String command) {
        ...
        try {
            ProcessBuilder pb = new ProcessBuilder("bash", "-c", command)
                    .directory(sandboxWorkDir.toFile())
                    .redirectErrorStream(false);
            ...
            if (!finished) {
                process.destroyForcibly();
                return SandboxResult.failure("Command execution timed out", -1,
                        System.currentTimeMillis() - startTime);
            }
